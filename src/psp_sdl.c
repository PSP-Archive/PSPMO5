...
psp_sdl_display_splash()
{
  ...
  int x = 240 - ((2 + strlen(PSPMO5_VERSION)) * 8 / 2);
  int y = 272 - 16;
  int col = psp_sdl_rgb(0x80, 0x80, 0x80);

  psp_sdl_blit_splash();
  psp_sdl_print(x, y, PSPMO5_VERSION, col);
  psp_sdl_flip();
...
psp_sdl_save_thumb_png(SDL_Surface* my_surface, char* filename)
{
  ...
  for (y = 0; y < SNAP_HEIGHT; y++) {
    u16* scan_pixel = src_pixel + (y*MO5_HEIGHT/SNAP_HEIGHT)*MO5_WIDTH;
    for (x = 0; x < SNAP_WIDTH; x++) {
      *dst_pixel++ = scan_pixel[x*MO5_WIDTH/SNAP_WIDTH];
    }
  }
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", MO5.mo5_home_dir, MO5.psp_screenshot_id++);
  if (MO5.psp_screenshot_id >= 10) MO5.psp_screenshot_id = 0;
...
# endif
  ...
  for (Index = 0; Index < MO5_MAX_SAVE_STATE; Index++) {
    MO5.mo5_save_state[Index].surface = 
