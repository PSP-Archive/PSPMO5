...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    ...
    } else if (!strcasecmp(pszExt, ".k7")) {
      return FMGR_FORMAT_K7;
    } else if (!strcasecmp(pszExt, ".fd")) {
      return FMGR_FORMAT_FD;
...
# endif
    ...
    psp_sdl_back2_rectangle(x, y, 250, rows * 10);
    if (display_thumb) {
      psp_sdl_blit_thumb(280, 50, save_surface);
    }
...
static char user_filedir_k7[PSP_FMGR_MAX_NAME];
static char user_filedir_fd[PSP_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format)
{
  ...
  if (first) {
    ...
    strcpy(user_filedir_k7 , user_filedir_kbd);
    strcpy(user_filedir_fd , user_filedir_kbd);
    strcpy(user_filedir_snap, user_filedir_kbd);
    strcat(user_filedir_kbd, "/kbd/");
    strcat(user_filedir_set, "/set/");
    strcat(user_filedir_cht, "/cht/");
    strcat(user_filedir_rom, "/roms/");
    strcat(user_filedir_k7 , "/k7/");
    strcat(user_filedir_fd , "/fd/");
  ...
  if (format == FMGR_FORMAT_ROM) user_filedir = user_filedir_rom;
  else
  if (format == FMGR_FORMAT_K7) user_filedir = user_filedir_k7;
  else
  if (format == FMGR_FORMAT_FD) user_filedir = user_filedir_fd;
  else                          user_filedir = user_filedir_snap;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_ROM) /* Rom */ error = mo5_load_rom(user_filename, 1);
      }
      else 
      {
        if (file_format == FMGR_FORMAT_STATE) /* State */ error = mo5_load_state(user_filename);
        else
        if (file_format == FMGR_FORMAT_ROM) /* Rom */ error = mo5_load_rom(user_filename, 0);
        else
        if (file_format == FMGR_FORMAT_K7) /* K7 */ error = mo5_load_k7(user_filename);
        else
        if (file_format == FMGR_FORMAT_FD) /* FD */ error = mo5_load_fd(user_filename);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = mo5_load_file_settings(user_filename);
        else
        if (file_format == FMGR_FORMAT_CHT) /* Cheat */  error = mo5_load_file_cheat(user_filename);
      }
    }
