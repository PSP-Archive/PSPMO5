...
# define MENU_REWIND_K7    1
# define MENU_COMMAND      2

# define MENU_LOAD_SLOT    3
# define MENU_SAVE_SLOT    4
# define MENU_DEL_SLOT     5

# define MENU_HELP         6

# define MENU_LOAD_ROM     7
# define MENU_LOAD_DISK    8
# define MENU_LOAD_K7      9
# define MENU_LOAD_STATE  10
...
# define MAX_MENU_ITEM (MENU_EXIT + 1)
  ...
  static int k7_device_loaded = 0;

#define MAX_COMMAND 5

  static char* command_name[MAX_COMMAND] = {
    "AUTO", "RUN", "LOAD", "LOADM", "DIR"
  };
...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", MO5.mo5_save_name);
...
psp_display_screen_menu(void)
{
  ...
  //if (psp_menu_dirty) 
  {
    ...
    psp_sdl_back2_print( 330, 6, " R: Cold Reset ", PSP_MENU_NOTE_COLOR);
  ...
  for (menu_id = 0; menu_id < MAX_MENU_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SCREENSHOT) {
      sprintf(buffer,"%d", MO5.psp_screenshot_id);
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(165, y, buffer, color);
    } else
    if (menu_id == MENU_REWIND_K7) {
      sprintf(buffer,"%d", psp_mo5_get_k7_index());
  ...
  for (slot_id = 0; slot_id < MO5_MAX_SAVE_STATE; slot_id++) {
    if (slot_id == cur_slot) color = PSP_MENU_SEL2_COLOR;
    else                     color = PSP_MENU_TEXT_COLOR;

    if (MO5.mo5_save_state[slot_id].used) {
      ScePspDateTime* my_date = &MO5.mo5_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (MO5.mo5_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(280, 50, MO5.mo5_save_state[cur_slot].surface);
  } else {
    psp_sdl_blit_thumb(280, 50, thumb_surface);
  }
...
psp_main_menu_hard_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Hard Reset MO5 !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  psp_mo5_hard_reset();
  reset_save_name();
...
psp_main_menu_soft_reset(void)
{
  /* Reset ! */
  psp_display_screen_menu();
  psp_sdl_back2_print(140, 160, "Soft Reset MO5 !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  psp_mo5_soft_reset();
  reset_save_name();
...
{
  char *RunName;
  RunName = psp_run_search(MO5.mo5_save_name);
  if (RunName != (char *)0 ) {
    psp_kbd_run_command(RunName);
  } else
  if (MO5.mo5_disk_mode && k7_device_loaded) {
...
psp_main_menu_cmd_auto(void)
{
  ...
  if (MO5.mo5_disk_mode && k7_device_loaded) {
    ...
    psp_kbd_run_command("RUN\"AUTO.BAT\"\n");
  }
}
...
{
  switch (cur_command) {
    case 0: psp_main_menu_cmd_auto(); break;
    case 1: psp_main_menu_cmd_run();  break;
    case 2: psp_main_menu_cmd_load(); break;
    case 3: psp_main_menu_cmd_loadm(); break;
    case 4: psp_main_menu_cmd_dir(); break;
  }
}
...
{
  if (MO5.mo5_save_state[cur_slot].used) {
    return mo5_snapshot_load_slot(cur_slot);
  }
  return -1;
}
...
{
  return mo5_snapshot_save_slot(cur_slot);
}
...
psp_main_menu_load_slot()
{
  ...
  error = mo5_snapshot_load_slot(cur_slot);
...
{
  int error;
  error = mo5_snapshot_save_slot(cur_slot);
...
psp_main_menu_del_slot()
{
  ...
  error = mo5_snapshot_del_slot(cur_slot);
...
psp_main_menu_rewind_k7()
{
  ...
  error = psp_mo5_rewind_k7();

  psp_display_screen_menu();
  psp_sdl_back2_print(140, 160, "K7 rewinded !", 
                     PSP_MENU_NOTE_COLOR);
  psp_sdl_flip();
  sleep(1);
}
...
{
  if (step == 1) {
    cur_slot++; if (cur_slot >= MO5_MAX_SAVE_STATE) cur_slot = 0;
  } else if (step == -1) {
    cur_slot--; if (cur_slot < 0) cur_slot = MO5_MAX_SAVE_STATE - 1;
  }
}
...
psp_main_menu_editor()
{
  ...
  sprintf(TmpFileName, "%s/txt/%s.txt", MO5.mo5_home_dir, MO5.mo5_save_name);
...
# endif
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_K7    : psp_menu_dirty = 1;
                               if (psp_main_menu_load(FMGR_FORMAT_K7)) {
                                 k7_device_loaded = 1;
                                 cur_menu_id = MENU_COMMAND;
                               }
                               old_pad = new_pad = 0;
                               psp_menu_dirty = 1;
        break;              
        ...
        case MENU_LOAD_DISK : psp_menu_dirty = 1;
                              if (psp_main_menu_load(FMGR_FORMAT_FD)) {
                                k7_device_loaded = 0;
                                cur_menu_id = MENU_COMMAND;
                              }
                              old_pad = new_pad = 0;
